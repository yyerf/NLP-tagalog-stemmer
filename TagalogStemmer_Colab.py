"""
TAGALOG STEMMER - Google Colab Version
======================================
This stemmer removes affixes (prefixes, infixes, suffixes) from Tagalog words
to extract the root word.

Usage in Google Colab:
1. Upload this file and validation.txt to Colab
2. Run the script
3. Enter Tagalog words when prompted
4. Type 'exit' or 'quit' to stop

Linguistic Rules Implemented:
- Prefixation (1-7 syllables) with hyphenation and morphophonemic rules
- Suffixation (/-in/, /-an/, /-hin/, /-han/) with phoneme restoration
- Infixation (/-in-/, /-um-/) after first consonant
"""
...
def clean_duplication(token, DUPLICATE):
    ...
    if len(split) == 2 and token != 'kama-kama':
        if '-' in token:
            if split[0] == split[1] and count_vowel(split[0]) >= 2:
                DUPLICATE.append(split[0])
                return split[0]

            elif split[0][1:] == split[1][1:] and len(split[0][1:]) > 1 \
                and check_consonant(split[0][0:1]) and check_consonant(split[1][0:1]):
                DUPLICATE.append(split[0][0])
                return split[1]

            elif split[1] == 'hin':
                if check_consonant(split[0][-1]):
                    DUPLICATE.append('han')
                    return split[0]

            elif len(split[1]) == 2:
                if check_consonant(split[0][-1]) and check_consonant(split[1][0]):
                    DUPLICATE.append(split[1])
                    return split[0]

            else:
                if not check_validation(split[0]) and check_validation(split[1]):
                    return split[1]
...
def change_letter(token, position, letter):
    try:
        temp = list(token)
...
def clean_stemmed(token, CLEANERS, REPITITION):
    ...
    if check_validation(token) or PASS_FLAG == False:
        return token
